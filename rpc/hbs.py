...
def get_hbs_builtinmetrics(data):
    """
    send formated data to transfer via rpc
    Args:
        data (dict): {}
    ...
    for i in range(3):
        try:
            res = rpc.call('Agent.BuiltinMetrics', data)
        except Exception as e:
            logging.warn("call (%s) Agent.BuiltinMetrics failure, times: %s -> msg: %s" %
                        (addr, i, e))
            continue
        return res
    logging.error("get_hbs_builtinmetrics %s to hbs (%s) failure" % (data, addr))
